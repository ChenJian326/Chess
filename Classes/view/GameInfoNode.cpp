...
bool GameInfoNode::init()
{
	...
	_pcInfo = Label::createWithSystemFont(Config::GBKToUTF8("PC：红色棋子"), "微软雅黑", 18);
	_pcInfo->setAnchorPoint(Vec2(1, 0));
	_pcInfo->setPosition(Vec2(VisibleRect::rightBottom().x, VisibleRect::leftBottom().y + spacing));

	_playerInfo = Label::createWithSystemFont(Config::GBKToUTF8("玩家：黑色棋子"), "微软雅黑", 18);
	...
	_currentInfo = Label::createWithSystemFont("", "微软雅黑", 18);
